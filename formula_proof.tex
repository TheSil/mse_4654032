...
\]
where $D$ runs over negative fundamental discriminants dividing $n$, $h_{\mathbb{Q}(\sqrt D)}$ is the class
number and $w_D=|\mathcal{O}_{\mathbb{Q}(\sqrt D)}^\times|\in\{2,4,6\}$ the number of units of $\mathbb{Q}(\sqrt D)$,
and $\leg{D}{p}$ is the Kronecker symbol.
\end{theorem}
...
Inserting $A$ from \eqref{eq:Aval} and $B$ from \eqref{eq:Bval} into the master decomposition
\eqref{eq:master} gives
\[
S_n=\binom n2-\frac{n(m-1)}{2}
-2n\sum_{\substack{D<0\ \text{fund.}\\ D\mid n}}\frac{h_{\mathbb{Q}(\sqrt D)}}{w_D}\sum_{D\cdot s^2\mid n}s\prod_{p\mid s}\Bigl(1-\tfrac1p\leg Dp\Bigr),
\]
which is Theorem~\ref{thm:main} (with $\binom n2=\frac{n(n-1)}2$). \qed

\subsection*{Two remarks}
\emph{An inequality for free.} Every class number satisfies $h_{\mathbb{Q}(\sqrt D)}\ge1$, every Euler factor
$1-\tfrac1p\leg Dp\in\{1-\tfrac1p,\,1,\,1+\tfrac1p\}$ is positive, and $m\ge1$; hence both corrections are
$\ge0$ and $S_n\le\binom n2$, with equality iff $n$ is squarefree and has no divisor $\equiv3\pmod4$.

\emph{Where the depth sits.} Steps 1--4 of $B$, and all of $A$, are elementary. Only two classical theorems are
